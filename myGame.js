...
firstPlayerName.innerHTML = firstPlayerPrompt;
secondPlayerName.innerHTML = secondPlayerPrompt;

function playerName() {
  if (firstPlayerPrompt != "" && secondPlayerPrompt != "") {
    player1.innerHTML =
      firstPlayerPrompt + " " + "time of clicked for successful draw";
    player2.innerHTML =
      secondPlayerPrompt + " " + "time of clicked for successful draw";
  } else {
  ...
  if (firstPlayerPrompt == "" || null) {
    firstPlayerPrompt = "Player1";
  }
  if (secondPlayerPrompt == "" || null) {
    secondPlayerPrompt = "Player2";
  }
}
...
function playLudo() {
  ...
  if (firstOutput.innerHTML == 6 && secondOutput.innerHTML == 6) {
    scdDisplayTxt.innerHTML =
      " You've successfully found double 6 after clicking in " +
      counter +
      " times";
    let display = " Your output is ";
    fstDisplayTxt.innerHTML = display;
    alert(
      "Hey! save your count times or time of clicked as " +
        counter +
        " before the next player start to click "
    );
  ...
  document.getElementById("displayTxt3").textContent =
    "You Just Clicked " + counter + " Times";
}
...
function save() {
  if (firstOutput.innerHTML != 6 || secondOutput.innerHTML != 6) {
    alert(
      "You've not gotten anything to save. \nPlay the game untill you found double 6 \nThanks"
    );
  } else if (input1.value != check.innerHTML) {
    alert(
      "Kindly please check what you are inputting and makesure that your total number of click for a successful draw is the same as what your are inputting\nYour input should be:\n\n" +
        check.innerHTML
    );
  } else {
...
function generate() {
  ...
  if (myinput.includes(geninputval.value)) {
    totalValue1.innerHTML =
      parseInt(totalValue1.innerHTML) + parseInt(geninputval.value);
    geninputval.value = count;
  } else if (myinput == "") {
    alert(
      "You've not gotten anything on your 'time of clicked for successful draw' Play the game and input your time of clicked there and then come back to generate some value here"
    );
  } else {
    alert(
      "Kindly please check the values in your 'time of clicked for successful draw' and makesure what you are inputting match a value there\n" +
...
function generate2() {
  ...
  if (myinput2.includes(geninputval2.value)) {
    totalValue2.innerHTML =
      parseInt(totalValue2.innerHTML) + parseInt(geninputval2.value);
...
function whoWin() {
  ...
  if (totalValue1.innerHTML < totalValue2.innerHTML) {
    ...
    displayCongratMsg.innerHTML =
      "No one is the winner, you both have the same scores, play the game for atleast one more time to decide the winner";
    congratImage.style.display = "none";
    gif.style.backgroundImage = "url(pic/congrats33.gif)";
    counterincrease += 0;
    counterincrease2 += 0;
  ...
  if (firstcounter.innerHTML <= 0 && secondcounter.innerHTML <= 0) {
    alert(
      "Winner can not be decided untill you generate user numbers of click.\nClick the 'Click To Play' button to play and get the required values"
    );
  }
}
...
function whyClick() {
  alert(
    "The save button is used to save each of your time of clicked or click time for a successful draw and they are entered into the space provided at the side of the save button.\n\nThe generate button is used to Input each digit from your time of clicked, and for every digit you enter, click the Genearte button to decide the winner and once you are done inputing them, click the Winner button to see the winner. \n\nNote: The Reset button is used to clear the generated value"
  );
}

function changeName() {
  newfirstPlayerName = prompt(
    "Do you really want to change your name from, " +
      firstPlayerName.innerHTML +
      " to another name? What's your new name as player1"
  );
  newsecondPlayerName = prompt(
    "Do you really want to change your name from, " +
      secondPlayerName.innerHTML +
      " to another name? Whats your new name as player2"
  );
  firstPlayerName.innerHTML = newfirstPlayerName;
  secondPlayerName.innerHTML = newsecondPlayerName;
  if (newfirstPlayerName != "" && newsecondPlayerName != "") {
    player1.innerHTML =
      newfirstPlayerName + " " + "time of clicked for successful draw";
    player2.innerHTML =
      newsecondPlayerName + " " + "time of clicked for successful draw";
  }
  if (newfirstPlayerName == null || newsecondPlayerName == null) {
    ...
    confirm(
      "Since you cancel and decided not to input any name, your name has been Reset back to default. \nIf you really want to change your name, kindly re-click 'Change Name' button and input your new name. Thanks"
    );
    player1.innerHTML =
      firstPlayerName.innerHTML + " " + "time of clicked for successful draw";
    player2.innerHTML =
      secondPlayerName.innerHTML + " " + "time of clicked for successful draw";
  }
  if (newfirstPlayerName == "" || newsecondPlayerName == "") {
    firstPlayerName.innerHTML = "player1"; //firstPlayerPrompt//"player1";
    secondPlayerName.innerHTML = "player2"; //secondPlayerPrompt//"player2";
    confirm(
      "The name space can not be empty and since you decided not to input any name, your name has been Reset back to default. \nIf you really want to change your name, kindly re-click 'Change Name' button and input your new name. Thanks"
    );
    player1.innerHTML =
      firstPlayerName.innerHTML + " " + "time of clicked for successful draw";
    player2.innerHTML =
